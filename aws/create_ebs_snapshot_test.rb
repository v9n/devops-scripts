...
module Aws
  ...
  module Ebs
    ...
    class TestCreator < Minitest::Test
      extend Minitest::Spec::DSL

      def setup
        @cleaner = SnapshotCreator.new 'aws'
      end
      ...
      def test_find_tag_volume
         mock = MiniTest::Mock.new
         mock.expect(:call, SNAPSHOT_RESPONSE, ["aws ec2 describe-volumes"])
         Shell.stub(:run, mock, ["aws ec2 describe-volumes"]) do
      ...
      def test_create
        mock = MiniTest::Mock.new
         mock.expect(:call, "", ["aws ec2 create-snapshot --snapshot-id snap-foo"])
         Shell.stub(:run, mock, ["aws ec2 create-snapshot --snapshot-id snap-foo"]) do
      ...
      def test_passing_attribute
        cleaner = SnapshotCreator.new 'aws --profile test'
